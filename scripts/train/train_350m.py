"""
Pretraining Script for Expera 350M Model.

Usage:
    torchrun --nproc_per_node=8 scripts/train/train_350m.py --data data/mixture --output checkpoints/expera-350m --epochs 3
"""
...
class GPT350M(nn.Module):
    """350M parameter GPT model."""

    def __init__(self, config: GPT350MConfig):
        """Initialize model."""
        super().__init__()
        self.config = config
...
class TextDataset(Dataset):
    ...
    def __init__(self, data_path: Path, seq_length: int = 2048, vocab_size: int = 32000):
    ...
    def _load_data(self) -> None:
        all_tokens = []
        for file in self.data_path.glob("*.jsonl") if self.data_path.exists() else []:
            try:
        ...
        self.data = all_tokens if all_tokens else [ord(c) % self.vocab_size for c in "def hello():\n    print('Hello!')\n" * 50]
...
class TrainingConfig:
    ...
    output_path: Path = Path("checkpoints/expera-350m")
    seq_length: int = 2048
    vocab_size: int = 32000
...
def save_checkpoint(model, optimizer, config: TrainingConfig, epoch: int, step: int, scaler=None) -> None:
    checkpoint = {
        "epoch": epoch,
        "step": step,
...
def main() -> None:
    ...
    parser.add_argument("--output", default="checkpoints/expera-350m")
    ...
    for epoch in range(config.epochs):
        ...
        for batch in dataloader:
            ...
            if step % config.log_interval == 0:
                logger.info(f"Epoch {epoch} Step {step} Loss: {loss.item() * config.gradient_accumulation:.4f}")

        scheduler.step()
        logger.info(f"Epoch {epoch} completed. Avg Loss: {total_loss / step:.4f}")

        if torch.cuda.is_available():
            save_checkpoint(model.module, optimizer, config, epoch, 0, scaler)
